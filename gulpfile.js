...
gulp.task('clean', () => {
    return gulp.src(gp_config.paths.devFiles, {read: false})
        .pipe(gp_if(!gp_config.dev, gp_clean()))
});

gulp.task('minify-css', () => {
    return gulp.src(gp_config.paths.cssFiles)
        .pipe(gp_if(gp_config.dev, gp_sourcemaps.init()))
        .pipe(gp_concat(gp_config.paths.outCssMin))
        .pipe(gp_cssnano(gp_config.minifyCSSOpts))
        .pipe(gp_if(gp_config.dev, gp_sourcemaps.write(gp_config.paths.cwd)))
        .pipe(gulp.dest(gp_config.paths.appDir + '/scripts'))
});

gulp.task('minify-js', () => {
    return gulp.src(gp_config.paths.jsFiles)
        .pipe(gp_if(gp_config.dev, gp_sourcemaps.init()))
        .pipe(gp_concat(gp_config.paths.outJsMin))
...
gulp.task('minify-html', () => {
    return gulp.src(gp_config.paths.htmlFiles)
        .pipe(gp_htmlmin(gp_config.minifyHTMLOpts))
        .pipe(gulp.dest(gp_config.paths.appDir))
});

gulp.task('copy-fonts', () => {
    return gulp.src(gp_config.paths.fontFiles)
        .pipe(gulp.dest(gp_config.paths.appDir + '/fonts'))
});

gulp.task('copy-icons', () => {
    return gulp.src(gp_config.paths.iconFiles)
        .pipe(gulp.dest(gp_config.paths.appDir))
});
...
gulp.task('lint', ['prepare'], () => {
    ...
    return gulp.src([
        ...
        .pipe(gp_eslint.format('json', fs.createWriteStream(gp_config.paths.outDir + '/lint-report.json')))
...
gulp.task('test', ['lint'], (done) => {
    ...
    exec('./node_modules/karma/bin/karma start karma.conf.js --single-run', (error, stdout, stderr) => {
        ...
        var testResultsFile = gp_config.paths.outDir + '/coverage/report.json';
        var testResults = {
...
gulp.task('doc', () => {
    return gulp.src([
        ...
        .pipe(gp_jsdoc3(apy_config.gulp.jsdoc3));
});
